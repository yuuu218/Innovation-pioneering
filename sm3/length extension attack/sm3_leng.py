...
def sm3(msg):
    ...
    return res[2:]
def sm3_leng():
    a=0x9172442d7da8a0600a
    b=0xe38dee4db0fb0e
    # 对a+fillFunction+b进行hash
    m1=fillFunction(a)
    left = (len(hex(b)) - 2) * 4
    r1 = (int(m1, 2) << left) + b
    x1 = sm3(r1)
    ...
    # 先对a进行hash，将得到的值作为IV对b+fillFunction+ll进行hash
    cc = sm3(a)
    ivv= int(cc, 16)
    m2 = fillFunction(b)
    ll = len(m2)
    ...
    r2 = int(m2, 2) + ll
    for i in hex(r2)[2:]:
        r2_ = r2_ + '{:04b}'.format(int(i, 16))
    x2 = hex(int(IterFunction_(r2_,ivv), 2))[2:]
    print("x2=", x2)
    if (x1 == x2):
        print("成功")
    else:
