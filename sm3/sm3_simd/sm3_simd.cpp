...
void sm3_compress_blocks_simd(uint32_t digest[8], const uint8_t* data, size_t blocks)
{
    uint32_t A;
    uint32_t B;
    uint32_t C;
...
void sm3_update_simd(SM3_CTX* ctx, const uint8_t* data, size_t data_len)
{
    size_t blocks;
    ...
    if (ctx->num)
    {
        ...
        else
        {
            ...
            sm3_compress_blocks_simd(ctx->digest, ctx->block, 1);
            ctx->nblocks++;
            data += left;
            data_len -= left;
        }
    ...
    sm3_compress_blocks_simd(ctx->digest, data, blocks);
    ctx->nblocks += blocks;
...
void sm3_finish_simd(SM3_CTX* ctx, uint8_t* digest)
{
    int i;
...
void sm3_digest_simd(const uint8_t* msg, size_t msglen,
    uint8_t dgst[SM3_DIGEST_SIZE])
{
    SM3_CTX ctx;
    sm3_init(&ctx);
    sm3_update_simd(&ctx, msg, msglen);
    sm3_finish_simd(&ctx, dgst);
}
...
int main()
{
    ...
    int d1 = input_str.size();
    unsigned char* m1 = (unsigned char*)input_str.c_str();
    unsigned char res1[32];
    printf("%s\n", m1);
    LARGE_INTEGER StartingTime1, EndingTime1, ElapsedMicroseconds1;
    LARGE_INTEGER Frequency1;
    QueryPerformanceFrequency(&Frequency1);  //
    QueryPerformanceCounter(&StartingTime1);
    sm3_digest(m1, d1, res1);
    QueryPerformanceCounter(&EndingTime1);
    ElapsedMicroseconds1.QuadPart = EndingTime1.QuadPart - StartingTime1.QuadPart;
    double timeSpend1 = (ElapsedMicroseconds1.QuadPart * 1000.0) / Frequency1.QuadPart;
    dump_buf(res1, 32);
    printf("时间为%gms\n", timeSpend1);
    int d2 = input_str.size();
    unsigned char* m2 = (unsigned char*)input_str.c_str();
