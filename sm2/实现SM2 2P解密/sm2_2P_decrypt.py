...
class SM2(ECC):
    ...
    def encrypt(self, M, PB, k=None):
        ...
        return True, join_bytes([C1, C2, C3]),self.Jacb_multiply(k, self.G),to_int(M) ^ t,C3
...
def twoP_decrypt():
    ...
    P1 = sm2_A.Jacb_multiply(d1, G)
    P2=sm2_B.Jacb_multiply(d2, G)
    sm2_A = SM2(*demo_para(), '', d1, P1)
    sm2_B = SM2(*demo_para(), '', d2, P2)
    d=get_inverse(d1*d2,n)-1
    P = sm2_A.Jacb_multiply(d, G)
    res, C, C1, C2, C3 = sm2_A.encrypt(M, P, k)

    T1=0
    if C1!=0:
        T1=sm2_A.Jacb_multiply(get_inverse(d1,n),C1)
    T2 = sm2_B.Jacb_multiply(get_inverse(d2, n), T1)
    C1fan = sm2_A.minus(C1)
    x2,y2=sm2_A.add(T2,C1fan)
    klen = len(C) - (sm2_A.keysize << 1) - HASH_SIZE << 3
    t = to_int(KDF(join_bytes([x2, y2]), klen))
    M_=to_byte(C2 ^ t, klen >> 3)
    u=sm3(join_bytes([x2, M_, y2]))
    if u==C3:
        print("解密得：" , M_.decode())
